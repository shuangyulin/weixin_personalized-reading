		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="任务类型" prop="renwuleixing" >
					<el-input v-model="ruleForm.renwuleixing" placeholder="任务类型" clearable  :readonly="ro.renwuleixing"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="任务类型" prop="renwuleixing" >
					<el-input v-model="ruleForm.renwuleixing" placeholder="任务类型" readonly></el-input>
				</el-form-item>
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.renwuneirong = this.ruleForm.renwuneirong.replace(reg,'../../../ssmixgl9940/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `renwutijiao/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.renwutijiaoCrossAddOrUpdateFlag = false;
											this.parent.search();
